...
def ikinematics(px_value, py_value, pz_value):
    ...
    costheta3 = ((px_value * m.cos(theta1) + py_value * m.sin(theta1)) ** 2 + (d1 - pz_value) ** 2 - a2 ** 2 - a3 ** 2) / (2 * a2 * a3)
    sintheta3 = m.sqrt(1 - (costheta3 ** 2))
    theta3 = m.atan2(sintheta3,costheta3)

    costheta2 = (
            ((d1 - pz_value) * a3 * m.sin(theta3) + (px_value * m.cos(theta1) + py_value * m.sin(theta1)) * (a3 * m.cos(theta3) + a2))
